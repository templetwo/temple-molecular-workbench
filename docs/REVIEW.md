...
The Playwright suite passes **13 real-browser workflows** in Chrome: all six distinct molecule renders; representations and controls; undo/redo; element search, category filters and keyboard focus; atom addition; valid/invalid import and export; typing guards; numeric coordinate editing; atom-list bonding/deletion; an actual canvas drag with one history entry; reload persistence; 390px mobile behavior; and a forced WebGL-unavailable fallback. Desktop and mobile screenshots were inspected visually. These tests use a software-rendered browser and do not establish performance on mobile hardware or cross-browser support.
...
Use `npm test`, `npm run lint`, and `npm run build` to verify the current checkout. The repository's [CI workflow](../.github/workflows/ci.yml) reports current automated results. The test count above is a recorded result, not a claim that every browser interaction or every inherited scientific value has been verified.
